...
function review_applications_config_page()
{
	if($_GET['review'])
		tl_review_application_forms();
	
	else
    	tl_view_application_forms_needing_review();
}
...
/** Get the application forms that have 'review' from the table wp_applications_forms **/
function tl_get_applications_for_review() {
    global $wpdb;
    $applicationFormsTable = $wpdb->prefix ."application_forms";
    ...
    FROM ".$applicationFormsTable." WHERE ".$applicationFormsTable.".application_status='submitted'
    ORDER BY ".$applicationFormsTable.".date_created ASC");
    
    return $applicationForms;
}

function tl_view_application_forms_needing_review()
{
    global $wpdb;
    $applicationFormsTable = $wpdb->prefix . 'application_forms';
        ...
        <form method="post" action="" id="tl_form_action">
            <table class="widefat page fixed" cellpadding="0">
                ...
                <tr>
                    ...
                    <th class="manage-column"><?php _e("Borrower's Name")?></th>
                ...
                if($table)
                {
                    ...
                    foreach($table as $applicationForm)
                    {
                        ...
                        <tr class="<?php echo (ceil($i/2) == ($i/2)) ? "" : "alternate"; ?>">
                            ...
                            <td><?php echo $applicationForm->borrowers_name?></td>
                            <td><a href="?page=review_applications&amp;review=<?php echo $applicationForm->form_id?>" target="_blank">Review Application</a></td>
...
function tl_review_application_forms()
{
	global $wpdb;
    $applicationFormsTable = $wpdb->prefix ."application_forms";
    ...
    ?>
    	<div class="revolver_application_form">
			<form id="loan_application_form" method="post" action="?page=review_applications">
				<!-- Section 1 -->
