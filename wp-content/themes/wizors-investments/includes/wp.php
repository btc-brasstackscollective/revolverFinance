...
if ( !function_exists( 'wizors_investments_get_template_page_link' ) ) {
	//Handler of the add_filter('post_type_archive_link', 'wizors_investments_get_template_page_link', 10, 2 );
...
if ( !function_exists( 'wizors_investments_wp_title' ) ) {
	// Handler of the add_filter( 'wp_title', 'wizors_investments_wp_title');
	// Handler of the add_filter( 'wp_title_parts', 'wizors_investments_wp_title');
	// Handler of the add_filter( 'document_title_parts', 'wizors_investments_wp_title');
...
if (!function_exists('wizors_investments_get_blog_title')) {
	function wizors_investments_get_blog_title() {
		...
		if (is_front_page())
			...
			$title = sprintf( esc_html__( '%s', 'wizors-investments' ), single_cat_title( '', false ) );
		else if ( is_tag() )
			$title = sprintf( esc_html__( 'Tag: %s', 'wizors-investments' ), single_tag_title( '', false ) );
		else if ( is_tax() )
			$title = sprintf( esc_html__( '%s', 'wizors-investments' ), single_term_title( '', false ) );
	...
	function wizors_investments_get_post_categories($delimiter=', ', $id=false) {
		...
		if ( !empty( $categories ) ) {
			foreach( $categories as $category )
				$output .= ($output ? $delimiter : '') . '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . sprintf( esc_attr__( 'View all posts in %s', 'wizors-investments' ), $category->name ) . '">' . esc_html( $category->name ) . '</a>';
		}
		return $output;
	}
}
	...
	function wizors_investments_get_post_terms($delimiter=', ', $id=false, $taxonomy='category') {
		...
		if ( !empty( $terms ) ) {
			foreach( $terms as $term )
				$output .= ($output ? $delimiter : '') . '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '" title="' . sprintf( esc_attr__( 'View all posts in %s', 'wizors-investments' ), $term->name ) . '">' . esc_html( $term->name ) . '</a>';
		}
		return $output;
	}
}
